...
def run_sequential(args, logger):
    ...
    while runner.t_env <= args.t_max:
        ...
        episode_batch, log_data = runner.run(test_mode=False)
        buffer.insert_episode_batch(episode_batch)
        ...
        with open('log_file.json', 'a') as file:
            for entry in log_data:
                # Convert each entry to a format that is JSON serializable
                print(entry)
                serializable_entry = {key: (value.tolist() if isinstance(value, np.ndarray) else value) for key, value in entry.items()}
                json.dump(serializable_entry, file)
                file.write('\n')
